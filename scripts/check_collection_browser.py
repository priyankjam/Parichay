...
with sync_playwright() as pw:
 ...
 expect(p.locator('.design-card')).to_have_count(49)
 for d in COLLECTION:
 ...
 expect(p.locator('.collection-options,[data-presentation],#design-options')).to_have_count(0);c.close()
 ...
 for width,height in [(320,568),(360,800),(390,844),(430,932),(768,1024),(1024,1366),(1280,800),(1440,900),(1920,1080)]:
  ...
  p.locator('[data-design-filter=cultural]').click();expect(p.locator('.design-card')).to_have_count(19)
  expect(p.locator('#document-preview')).to_have_attribute('data-state','ready',timeout=65000)
 ...
 p.locator('[data-step="9"]').click();p.locator('[data-template=craft-ganesha-ivory]').focus();p.keyboard.press('Enter');expect(p.locator('[data-template=craft-ganesha-ivory]')).to_have_attribute('aria-pressed','true');p.locator('[data-template=craft-ivory-saffron]').click();expect(p.locator('#document-preview')).to_have_attribute('data-state','ready',timeout=65000)
 ...
 checks.append({'keyboard_selection':True,'live_update':True,'standalone_variant':True,'no_personalization_controls':True});c.close();b.close()
