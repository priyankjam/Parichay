...
with app.app_context(),sync_playwright() as pw:
 ...
 # Match the current worker's shared semantic print layout.
 for number,design in enumerate(ordered,1):
  if design.get("retired"):continue
  ...
  for gender in ['male','female']:
   raw=sample(gender);raw['template']=design['id'];doc=Profile.parse(raw).document();page.set_content(build_document_html(doc),wait_until='load');page.evaluate('document.fonts.ready');page.evaluate(paginator)
